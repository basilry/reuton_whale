...
> 배포 URL: [https://whalescope.6esk.com](https://whalescope.6esk.com) (Vercel). 공개 Telegram 채널: [@whalescope_alertz](https://t.me/whalescope_alertz). Render에서 정보수집 pipeline / Telegram bot / Telethon listener 3개 서비스가 상시 가동됩니다.
...
- 최근 반영 보고서: v6 개선 (2026-04-19, 하이브리드 브리핑·Render 관측·UX 폴리싱), 체인 커버리지 확장 완료 (2026-04-20, XRP/TRX/BTC/DOGE + TG mirror observability + BTC fallback)
...
- `/admin`은 운영/관리 대시보드입니다. 정보수집 worker, Telegram worker, Google Sheets 적재 상태, 최신 브리핑, 시스템 상태를 확인하는 용도입니다.
...
| 정보수집 파이프라인 worker | `python -m src.pipeline.run_all` | Render Cron Job | 온체인/TG 데이터 수집, 시그널 생성, 뉴스/스토리/브리핑/헬스체크 cadence 실행, Sheets 저장, Telegram 브리핑 발송 | 주기 실행 |
...
| Next.js 운영 대시보드(`/admin`) | `npm run dashboard:dev` | Vercel | Google Sheets 데이터를 읽어 운영 화면/API 제공. listener 상태는 Sheets 연결 여부가 아니라 `telethon_listener` heartbeat 우선, `tg_whale_events` 최신 기록 보조 기준으로 표시 | 화면 확인 시 또는 Vercel 상시 배포 |
...
- 대시보드가 비어 있으면 대시보드 문제가 아니라 먼저 `python -m src.pipeline.run_all`이 Sheets에 `transactions`, `signals`, `daily_brief`, `system_log`를 쌓았는지 확인합니다.
...
v6(2026-04-19)과 체인 커버리지 확장(2026-04-20) 사이클에서 추가된 구조적 개선은 다음과 같습니다. 각 항목의 상세 변경 로그는 `docs/changelog.md`와 Obsidian 프로젝트 노트(`Projects/02015-WhaleScope/`)에 정리되어 있습니다.
...
- **하이브리드 브리핑 (full / incremental)**: KST 09/15/21시 slot은 전체 컨텍스트 full brief, 그 외 slot은 이전 brief 기반 incremental brief로 분기합니다. RSS top N + curated watchlist를 full slot에만 컨텍스트로 주입하고 JSONL 로그를 `data/brief_logs/`에 누적합니다. Sonnet 기준 월 비용이 약 $21에서 $9 수준으로 감소합니다.
...
11. 대시보드는 Google Sheets snapshot을 server-side에서 읽고, Upstash Redis SSE로 실시간 섹션을 브라우저에 push합니다.
...
Next.js 대시보드의 brief / news / watchlist / stories 4개 섹션은 Upstash Redis REST 기반 SSE로 브라우저에 push됩니다. 아래 3-gate가 모두 충족되어야 `실시간 연결됨` 상태가 되며, 하나라도 빠지면 `실시간 비활성` (standby)으로 표시됩니다.

| 변수 | 설명 |
|---|---|
| `WHALESCOPE_SSE_ENABLED` | SSE 기능 on/off feature flag. 기본값 `false`. 운영에서 켤 때 `true`. |
| `WHALESCOPE_REDIS_REST_URL` | Upstash Redis REST endpoint URL. |
| `WHALESCOPE_REDIS_REST_TOKEN` | Upstash Redis REST token. server-only secret. |

Vercel Production에서 Redis 연결이 활성화되지 않는 가장 흔한 원인은 이 3개 변수를 Preview 또는 Development 스코프에만 등록하고 Production 스코프에 누락한 경우입니다. Vercel 프로젝트의 Environment Variables 스코프를 반드시 확인하세요.

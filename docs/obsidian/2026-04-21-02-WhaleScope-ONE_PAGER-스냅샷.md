...
> **Day 10 사이클에서 해소된 항목 (참고)**: 이전 판본에서 "현재 제약"으로 잡혀 있던 (a) Google Sheets 429 / 다인스턴스 쿼터 경합, (b) 대시보드 초기 로딩 체감 지연, (c) 모바일 터치 타깃/레이아웃 overflow는 Day 10 폴리싱 사이클에서 구조적으로 해결되었습니다 (Redis L1+L2 2-tier 캐시, Phase A~F 성능 개선 `/` First Load JS 312→206 kB, WCAG 2.5.5 44×44 px 타깃 전수 교정). 상세는 §5 Day 10 섹션과 README 트러블슈팅을 참조하세요.
...
```text
  ...
  └───────┬──────────────────────────────┬─────────────────────┬──────────┘
   ...
   tg_mirror                                              + 김치 프리미엄
       ...
       │  keys: whalescope:sheet:*  (60s TTL)      │            │
       │    ↕ upsertWatchlistOverride 동시 무효화   │            │
       │  L1 cache: Next.js server Map (45s TTL)   │            │
       │  · graceful degrade: L2 실패 시 원본 호출  │            │
       └───────────────────────────────────────────┘            │
                         │                                      │
           ┌─────────────┼─────────────────────────┐            │
           ▼             ▼                         ▼            ▼

...
### Telegram bot `Conflict: terminated by other getUpdates request` (Render 재배포 직후)

Render bot 인스턴스 재배포 시 Telegram `getUpdates` long polling이 기존 인스턴스와 충돌하여 새 인스턴스가 반복적으로 Conflict 예외를 출력하고 크래시하는 현상이 발생할 수 있습니다. 원인은 (1) `app.run_polling(stop_signals=None)` 때문에 SIGTERM이 마스킹되어 구버전 프로세스가 좀비 상태로 polling을 붙잡고 있거나, (2) 시작 시점에 `deleteWebhook(drop_pending_updates=True)`가 호출되지 않아 기존 세션이 그대로 남거나, (3) `add_error_handler`가 없어 Conflict가 reset loop로 이어지는 경우입니다. 임시 조치는 Render 서비스를 수동으로 Restart하여 이전 프로세스를 확실히 정리하는 것이며, 근본 조치로는 `scripts/run_bot.py`에서 `stop_signals=None` 제거 + startup에서 `delete_webhook(drop_pending_updates=True)` + `app.add_error_handler` 등록이 필요합니다.

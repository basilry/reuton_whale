...
class SheetsClient:
    ...
    def get_active_watchlists(self) -> list[dict]:
        try:
            ws = self._worksheet(TAB_WATCHLIST)
            all_values = ws.get_all_values()
            if len(all_values) <= 1:
                return []
            active_col = WATCHLIST_HEADERS.index("active")
            return [
                row_to_dict(row, WATCHLIST_HEADERS)
                for row in all_values[1:]
                if row[active_col].lower() == "true"
            ]
        except gspread.exceptions.APIError as e:
    ...
    def upsert_watchlist(self, user_id: int, username: str, coins: list[str]) -> None:
        try:
            ...
            row_data = dict_to_row(entry, WATCHLIST_HEADERS)

            if target_row:
                cell_range = f"A{target_row}:{chr(64 + len(WATCHLIST_HEADERS))}{target_row}"
                ws.update(cell_range, [row_data])
                logger.info("Updated watchlist for user %d", user_id)
            else:
                ws.append_row(row_data, value_input_option="RAW")
                logger.info("Created watchlist for user %d", user_id)
        except gspread.exceptions.APIError as e:
            raise StorageError(f"Failed to upsert watchlist: {e}") from e
